...
``LightRAGAdapter`` that satisfies ``KnowledgeSearchPort``. It wires:
    ...
    settings -> LightRAGAdapter -> RagSearchTool

It does NOT import LightRAG directly -- only ``LightRAGAdapter`` and
``RagSearchTool``. No Agent is created (Stage 4).
"""
...
def build_rag_search_tool(*, adapter: LightRAGAdapter) -> RagSearchTool:
    """Bind the concrete adapter to a RagSearchTool via the Port."""
    return RagSearchTool(search_port=adapter)
